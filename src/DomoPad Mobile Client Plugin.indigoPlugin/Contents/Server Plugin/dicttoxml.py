...
def get_xml_type(val):
    ...
    if type(val).__name__ == 'NoneType':
        return 'null'
    if isinstance(val, dict) or isinstance(val, indigo.Dict):
        return 'dict'
...
def convert(obj, ids, attr_type, item_func, cdata, parent='root'):
    ...
    if isinstance(obj, dict) or isinstance(obj, indigo.Dict):
...
def convert_dict(obj, ids, parent, attr_type, item_func, cdata):
    ...
    for key, val in obj.items():
        ...
        elif isinstance(val, dict) or isinstance(val, indigo.Dict):
            if attr_type:
...
def convert_list(items, ids, parent, attr_type, item_func, cdata):
    ...
    for i, item in enumerate(items):
        ...
        elif isinstance(item, dict) or isinstance(item, indigo.Dict):
            if not attr_type:
                addline('<%s>%s</%s>' % (
                    item_name, 
